        ...
        __renderDirectData: ({ results }, { limit }) => {
            let body = results.data
                .map(v => {
                    return `
                        <tr>
                            <td style="width: 25%;">${v.perkiraan.perkiraan_no}</td>
                            <td style="width: 25%;">${v.perkiraan.perkiraan_name}</td>
                            <td style="width: 25%;">${v.debit}</td>
                            <td style="width: 25%;">${v.kredit}</td>
                        </tr>
                    `;
                }).join("");
...
const DaftarPerkiraanController = ((SET, UI) => {
    ...
    const __fetchDirectPerkiraan = (TOKEN, filter = {}, link = null) => {
        $.ajax({
            url: `${link === null ? SET.__apiURL() + 'cabang/get_daftarPerkiraan' : link}`,
            type: 'GET',
            dataType: 'JSON',
            data: filter,
