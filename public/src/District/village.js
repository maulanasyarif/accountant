        ...
        __renderDirectData: ({ results }, { limit }) => {
            let body = results.data
                .map(v => {
                    return `
                        <tr>
                            <td style="width: 50%;"><a href="${SET.__baseURL()}erwe/direct/${v.id}">${v.name}</a></td>
...
const VillageController = ((SET, UI) => {

    const __fetchDirectOrder = (TOKEN, filter = {}, link = null) => {
        $.ajax({
            url: `${link === null ? SET.__apiURL() + 'district/villages' : link}`,
            type: 'GET',
            dataType: 'JSON',
            data: filter,
    ...
    const __getDirectOrderId = (TOKEN, id, callback) => {
        $.ajax({
            url: `${SET.__apiURL()}district/erwe/${id}`,
            type: 'GET',
            dataType: 'JSON',
            headers: {
                'Authorization': `Bearer ${TOKEN}`,
