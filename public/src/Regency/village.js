...
const RegentController = ((SET, UI) => {
    ...
    const __fetchDirectOrder = (TOKEN, filter = {}, link = null, id) => {
        ...
        $.ajax({
            url: `${link === null ? SET.__apiURL() + `regency/villages/${id}` : link}`,
            type: 'GET',
            dataType: 'JSON',
            data: filter,
            beforeSend: SET.__tableLoader('#t_village', 7),
            headers: {
                'Authorization': `Bearer ${TOKEN}`
            },
            success: (res) => {
                $("#count_villages").text(res.total_all);
    ...
    const __clickDirectPagination = (TOKEN, filter = {}) => {
        $('#t_village').on('click', '.btn-pagination', function () {
