        ...
        __renderDirectData: ({ results }, { limit }) => {
            let body = results.data
                .map(v => {
                    return `
                        <tr>
                            <td style="width: 50%;">${v.district.name}</td>
...
const DistributionController = ((SET, UI) => {
    ...
    const __fetchDirectOrder = (TOKEN, filter = {}, link = null) => {
        $.ajax({
            url: `${link === null ? SET.__apiURL() + 'regency/district' : link}`,
            type: 'GET',
            dataType: 'JSON',
            data: filter,
    ...
    const __getRegency = (TOKEN, callback) => {
        $.ajax({
            url: `${SET.__apiURL()}regency/districts`,
            type: 'GET',
            dataType: 'JSON',
            headers: {
                'Authorization': `Bearer ${TOKEN}`,
    ...
    const __submitAdd = (TOKEN, filter) => {
        $("#form_add").validate({
            ...
            submitHandler: (form) => {
                $.ajax({
                    url: `${SET.__apiURL()}regency/storeDistribution`,
                    type: "POST",
                    dataType: "JSON",
                    data: $(form).serialize(),
                    beforeSend: (xhr) => {
