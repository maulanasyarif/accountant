...
const DistrictsController = ((SET, UI) => {

    const __fetchDirectDistrict = (TOKEN, filter = {}, link = null, id) => {
        var url = window.location.pathname;
        var id = url.substring(url.lastIndexOf('/') + 1);

        $.ajax({
            url: `${link === null ? SET.__apiURL() + `prov/districts/${id}` : link}`,
            type: 'GET',
            dataType: 'JSON',
            data: filter,
    ...
    const __resetDirectFilter = TOKEN => {
        $('#btn_direct_reset').on('click', function () {
            ...
            __fetchDirectDistrict(TOKEN, { limit: 10 })
        })
    }
    ...
    return {
        init: TOKEN => {
            ...
            __fetchDirectDistrict(TOKEN, direct_filter, null, id)
            __clickDirectPagination(TOKEN, direct_filter)
            __closeDirectFilter()
        },
    }
    
})(SettingController, DistrictsUI)